...
Лендинг:
C:\beerbankApp\BeerbankRepo\landingpage
...
6. ПРОПУСКАЕМ ЭТОТ ЭТАП В MVP. **Экран авторизации** (дизайн + Google OAuth) _Результат: при открытии приложения показывается экран логина. Google OAuth работает. После логина открывается главный экран. Logout работает._
7. ПРОПУСКАЕМ ЭТОТ ЭТАП В MVP **Онлайн-режим** (синхронизация с Supabase) _Результат: данные тянутся с сервера при открытии. После каждого действия данные сохраняются в Supabase. Одни и те же данные видны на телефоне и компьютере под одним аккаунтом._
8. ПРОПУСКАЕМ ЭТОТ ЭТАП В MVP **Офлайн-режим** (Dexie + PWA) _Результат: приложение работает без интернета. Действия сохраняются локально. При восстановлении связи данные синхронизируются с сервером без дублей._
9.  ✅ **Деплой на Vercel** - СДЕЛАНО, ДОСТУПНО НА HTTPS://BEERBANK.VERCEL.APP.
   Результат: приложение доступно по публичной ссылке. Можно открыть на телефоне и добавить на главный экран._
10.  ✅ **Лендинг** - СДЕЛАНО, ДОСТУПНО НА HTTPS://GETBEERBANK.VERCEL.APP _Результат: по корневому URL открывается страница с описанием приложения. Переход на сайт с приложением - по кнопке Download.
